...
def get_labeled_data(ids_list, data_list, label_file, l_id_col=1, l_label_col=None, l_first_row=3,
    ...
    _, temp_labels, temp_ids = dataloader([label_file], id_cols=l_id_col, label_cols=l_label_col,
...
def load_classifer_data(runner, data_list, labels_list, ids_list, dataset=None,
    ...
    if not create_train_valid:
        runner.classifier.load_dataset(dataset, data=data_list, labels=labels_list, ids=ids_list)
    else:
        train_ids, valid_ids = runner.classifier.create_train_and_valid(data=data_list, labels=labels_list, ids=ids_list,
...
if __name__ == "__main__":
    ...
    for rule in cur_run:
        ...
        if "label_file" in cur_params:
            data["all"], labels["all"], ids["all"] = get_labeled_data(**cur_params)
            classifier_runner = load_classifer_data(classifier_runner, data["all"], labels['all'], ids["all"], create_train_valid=True,
        ...
        for cur_dataset in datasets:
            all_classifications = []
            print("\nRunning on rule: {} - {}".format(rulename, cur_dataset))
            ...
            accuracy, incorrect_indices = calculate_accuracy(classifier_runner.classifier.dataset[cur_dataset]["preds"],
            ...
            for index in incorrect_indices:
                patient_id = classifier_runner.classifier.dataset[cur_dataset]["ids"][index]
                pred = classifier_runner.classifier.dataset[cur_dataset]["preds"][index]
                label = classifier_runner.classifier.dataset[cur_dataset]["labels"][index]
                match_obj = classifier_runner.classifier.dataset[cur_dataset]["matches"][index]
                score = classifier_runner.classifier.dataset[cur_dataset]["scores"][index]
                text = classifier_runner.classifier.dataset[cur_dataset]["data"][index]
            ...
            print("Incorrect Predictions: ", classifier_runner.classifier.dataset[cur_dataset]["preds"][incorrect_indices])
            ...
            if not os.path.exists(os.path.join("generated_data", rulename, cur_dataset)):
                os.makedirs(os.path.join("generated_data", rulename, cur_dataset))
            ...
            generate_error_report(os.path.join("generated_data", rulename, cur_dataset),template_directory, "{}".format(rulename),
            ...
            excel_path = os.path.join("generated_data", rulename, cur_dataset, rulename)
            conf_path = os.path.join("generated_data", rulename, cur_dataset)
